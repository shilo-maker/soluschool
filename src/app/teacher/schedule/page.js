...
export default function TeacherSchedulePage() {
  ...
  const fetchSchedules = async () => {
    try {
      setLoading(true);
      const response = await fetch('/api/schedules?teacherId=me');
      if (response.ok) {
        const data = await response.json();
  ...
  const getScheduleHeight = (schedule) => {
    ...
    // Each 30-minute slot is about 60px high (you can adjust this)
      ...
      <Container fluid dir="rtl" className="py-4">
        ...
        {schedules.length === 0 ? (
          <Alert variant="info">
            ...
            <br />
            פנה למנהל כדי להוסיף שיעורים למערכת השעות שלך.
          </Alert>
        ) : (
          <Card>
              ...
              <Card>
                ...
                <Card.Body>
                  <Alert variant="info" className="mb-0">
                    ...
                    <a href="/teacher/lessons" className="alert-link"> דף השיעורים</a>.
                  </Alert>
                </Card.Body>
              </Card>
            </Col>
        ...
        <Modal show={showModal} onHide={() => setShowModal(false)} dir="rtl">
          ...
          <Modal.Body>
            {selectedSchedule && (
              <>
                <div className="mb-3">
                  <strong>תלמיד/ה:</strong>
                  <div className="mt-1">
                    {selectedSchedule.student?.user?.firstName} {selectedSchedule.student?.user?.lastName}
                  </div>
                </div>
