...
export default function StudentCalendarPage() {
  ...
  const fetchLessons = async () => {
    try {
      ...
      const response = await fetch(`/api/lessons?studentId=me&startDate=${startDate}&endDate=${endDate}`);
      if (response.ok) {
        const data = await response.json();
  ...
  const convertLessonsToEvents = (lessonsData) => {
    const calendarEvents = lessonsData.map((lesson) => {
      ...
      return {
        id: lesson.id,
        title: `${lesson.teacher?.user?.firstName} ${lesson.teacher?.user?.lastName} - ${lesson.instrument}`,
        start,
        end,
        resource: lesson,
        style: {
  ...
  const handleNavigate = (date) => {
    ...
    fetch(`/api/lessons?studentId=me&startDate=${startDate}&endDate=${endDate}`)
      .then(res => res.json())
      .then(data => {
        setLessons(Array.isArray(data) ? data : []);
        convertLessonsToEvents(Array.isArray(data) ? data : []);
    ...
    <>
      ...
      <Container fluid dir="rtl" className="py-4">
        ...
        <Row className="mb-4">
          <Col>
            <h2>My Calendar</h2>
            <p className="text-muted">Visual overview of my lessons</p>
          </Col>
        </Row>
              ...
              <>
                  ...
                  <Row className="mb-3">
                    <Col md={12}>
                      <Alert variant="warning">
                        ...
                        <div className="mt-2">{selectedEvent.cancellationReason}</div>
                      </Alert>
                    </Col>
                  </Row>
                )}
                ...
                {selectedEvent.studentCheckIn && (
                  <Row className="mb-3">
                    <Col md={12}>
                      <Badge bg="success">
                        Checked in at {new Date(selectedEvent.studentCheckIn).toLocaleTimeString('en-US', {
                          hour: '2-digit',
                          minute: '2-digit',
                        })}
                      </Badge>
          ...
          <Modal.Footer>
            ...
            {selectedEvent && (
              <Button
                ...
                onClick={() => {
                  setShowModal(false);
                  router.push(`/student/lessons/${selectedEvent.id}`);
                }}
              >
                View Details
              </Button>
