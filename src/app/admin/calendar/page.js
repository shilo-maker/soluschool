...
export default function AdminCalendarPage() {
  ...
  const fetchLessons = async () => {
    try {
      ...
      const response = await fetch(`/api/lessons?startDate=${startDate}&endDate=${endDate}`);
      if (response.ok) {
        const data = await response.json();
  ...
  const convertLessonsToEvents = (lessonsData) => {
    const calendarEvents = lessonsData.map((lesson) => {
      ...
      return {
        id: lesson.id,
        title: `${lesson.teacher?.user?.firstName} ${lesson.teacher?.user?.lastName} - ${lesson.student?.user?.firstName} ${lesson.student?.user?.lastName} - ${lesson.instrument}`,
        start,
        end,
        resource: lesson,
        style: {
  ...
  const handleNavigate = (date) => {
    ...
    fetch(`/api/lessons?startDate=${startDate}&endDate=${endDate}`)
      .then(res => res.json())
      .then(data => {
        setLessons(Array.isArray(data) ? data : []);
        convertLessonsToEvents(Array.isArray(data) ? data : []);
      ...
      <Container fluid dir="rtl" className="py-4">
        ...
        <Row className="mb-4">
          <Col>
            ...
            <p className="text-muted">Visual overview of all lessons</p>
          </Col>
        </Row>
              ...
              <>
                ...
                <Row className="mb-3">
                  <Col md={6}>
                    ...
                    <div>
                      {selectedEvent.teacher?.user?.firstName} {selectedEvent.teacher?.user?.lastName}
                    </div>
                  </Col>
                  <Col md={6}>
                ...
                <Row className="mb-3">
                  <Col md={6}>
                    {selectedEvent.teacherCheckIn && (
                      <Badge bg="success">
                        Teacher checked in at {new Date(selectedEvent.teacherCheckIn).toLocaleTimeString('en-US', {
                          hour: '2-digit',
                          minute: '2-digit',
                        })}
                      </Badge>
                  ...
                  <Col md={6}>
                    {selectedEvent.studentCheckIn && (
                      <Badge bg="success">
                        Student checked in at {new Date(selectedEvent.studentCheckIn).toLocaleTimeString('en-US', {
                          hour: '2-digit',
                          minute: '2-digit',
                        })}
                      </Badge>
          ...
          <Modal.Footer>
            ...
            {selectedEvent && (
              <Button
                ...
                onClick={() => {
                  setShowModal(false);
                  router.push(`/admin/lessons/${selectedEvent.id}`);
                }}
              >
                View Details
              </Button>
